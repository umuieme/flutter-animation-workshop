...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Iterable<Widget> _buildPages() {
    ...
    for (int index = 0; index < location.length; index++) {
      var delta = selectedIndex.value - index;
      var alignment = Alignment.center.add(Alignment(delta * 0.5,
          0.0)); // this controls right and left alignment of not selected item
      var resizeFactor =
          (1 - ((delta.abs() * 0.26).clamp(0.0, 1.0))); // this controls width of height of selected and not selected page
      var opacity = 1 - min(delta.abs(), 1) * .6; //this controls opacity of not selected item

      pages.add(_buildPageItem(
        location[index],
        opacity: opacity,
        alignment: alignment,
